...
contract Tracking {
    ...
    function startShipment(address _sender, address _receiver, uint256 _index) public {
        ...
        ShipmentType storage shipmentType = shipmentTypes[_index];

        require(shipment.receiver == _receiver, "Invalid receiver.");
        require(shipment.status == ShipmentStatus.PENDING, "Shipment already in transit.");
        
        shipment.status = ShipmentStatus.IN_TRANSIT;
        shipmentType.status = ShipmentStatus.IN_TRANSIT;
    ...
    function completeShipment(address _sender, address _receiver, uint256 _index) public {
        ...
        require(!shipment.isPaid, "Shipment is already paid for.");

        shipment.status = ShipmentStatus.DELIVERED;
        shipmentType.status = ShipmentStatus.DELIVERED;

        shipmentType.deliveryTime = block.timestamp;
